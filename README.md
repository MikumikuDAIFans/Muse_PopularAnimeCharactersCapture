...
- 每周增量元数据同步与榜单刷新入口，支持 dry-run、日期窗口、resume、任务状态和榜单快照记录。
...
Docker Compose 会启动 PostgreSQL，并挂载 `./output` -> `/app/output`。这些运行时文件不会提交到 git。
...
- Danbooru metadata ingestion with PostgreSQL storage.
...
The compose file mounts `./output` to `/app/output` and starts PostgreSQL as the primary database.
